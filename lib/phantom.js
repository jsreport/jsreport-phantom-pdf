...
Phantom.prototype.init = function () {
  ...
  if (this.reporter.execution2) {
    var phantomjsPackage = require(this.reporter.execution2.resourcePath('phantomjsPackage.json'))
  ...
  return crawlAvailablePhantomVersions().then(function (versions) {
    ...
    if (self.reporter.compilation2) {
      if (self.definition.options.defaultPhantomjsVersion && self.definition.options.defaultPhantomjsVersion !== defaultPhantomjsVersion) {
        self.reporter.compilation2.resourceInTemp('phantomjs.exe', require(self.definition.options.phantoms[0].path).path)
        self.reporter.compilation2.resource('phantomjsPackage.json', path.join(path.dirname(require.resolve(self.definition.options.phantoms[0].path)), 'package.json'))
      } else {
        self.reporter.compilation2.resourceInTemp('phantomjs.exe', require('phantomjs').path)
        self.reporter.compilation2.resource('phantomjsPackage.json', path.join(path.dirname(require.resolve('phantomjs')), '../package.json'))
      }
    }
  })
}
...
Phantom.prototype.execute = function (request, response) {
  ...
  if (reporter.execution2) {
    request.template.phantom.phantomPath = reporter.execution2.resourceTempPath('phantomjs.exe')
  } else {
