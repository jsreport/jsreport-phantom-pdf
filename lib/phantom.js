...
Phantom.prototype.init = function () {
  ...
  return crawlAvailiblePhantomVersions().then(function (versions) {
    ...
    if (self.reporter.compilation) {
      // todo make it working with custom phantomjs
      self.reporter.compilation.resourceInTemp('phantomjs.exe', require('phantomjs').path)
      self.reporter.compilation.resourceInTemp('standaloneScript.js', path.join(path.dirname(require.resolve('phantom-html-to-pdf')), 'lib', 'scripts', 'standaloneScript.js'))
    }
  })
}
...
module.exports = function (reporter, definition) {
  ...
  if (reporter.execution) {
    ...
    definition.options.phantomPath = reporter.execution.resourceTempPath('phantomjs.exe')
  }
